...
$(function () {
    ...
    if (parentId == 9) {
        ...
        title = "图册";
        $("#current1").addClass("single-page-current");
    }
    else if (parentId == 11) {
        title = "周边环境";
        $("#current2").addClass("single-page-current");
    }
    $("#navTitle").text(title);
    $("#navTitle2").text(title);
    AlbumNext();
    var html = ""; var html2 = "";
            ...
            if (data.Status == "0") {
                ...
                $(data).each(function (i, item) {
                    if (item.LastAlbumTypeId != "") {
                        ...
                        $("#btnLast").text(item.LastAlbumTypeName).attr("href", "tpl-inner-gallery-g.html?typeId=" + item.LastAlbumTypeId + "&parentId=" + item.LastParentId);
                    }
                    else {
                        $("#btnLast").text("已经是第一页了").attr("href", "tpl-inner-gallery-g.html?typeId=" + typeId + "&parentId=" + parentId);
                    }
                    if (item.NextAlbumTypeId != "") {
                        $("#imgNext").attr("style", "background-image:url(" + item.NextCoverImage + ")");
                        $("#btnNext").text(item.NextAlbumTypeName).attr("href", "tpl-inner-gallery-g.html?typeId=" + item.NextAlbumTypeId + "&parentId=" + item.NextParentId);
                    }
                    else {
                        $("#btnNext").text("已经是最后一页了").attr("href", "tpl-inner-gallery-g.html?typeId=" + typeId + "&parentId=" + parentId);
                    }
                });
            }
        },
